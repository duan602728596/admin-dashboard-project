...
import type { GraphQLContext } from '../commonTypes';
...
import type { UserAddOrUpdateResponseData } from '../../src/services/graphql';
...
export interface UserRootValue {
  ...
  add(variableValues: UserAddVariableValues, context: GraphQLContext): UserAddOrUpdateResponseData;
  update(variableValues: UserUpdateVariableValues, contextValues: GraphQLContext): UserAddOrUpdateResponseData;
  logout(variableValues: null, context: GraphQLContext): UserAddOrUpdateResponseData;
}

export const userRootValue: UserRootValue = {
  // 用户登录
  ...
  // 添加新用户
  add(variableValues: UserAddVariableValues, contextValues: GraphQLContext): UserAddOrUpdateResponseData {
    if (!contextValues.token) throw new Error('需要先登录');
  ...
  // 退出
  logout(variableValues: null, contextValues: GraphQLContext): UserAddOrUpdateResponseData {
    return { success: true };
  }
};